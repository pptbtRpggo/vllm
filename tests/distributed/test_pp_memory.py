...
def _profile(devices, tp=1):
    return PPMemoryProfile(
        len(devices[0].layer_weights_bytes),
        len(devices) // tp,
        tp,
        _scope(),
        tuple(devices),
...
def _costs(n=32, speeds=(1, 1), comm=100):
    return [
        RankCost(r, n // len(speeds), speed, comm if r < len(speeds) - 1 else None, 10)
...
@pytest.mark.parametrize(
    "objective,overlap",
    [("latency", False), ("throughput", False), ("throughput", True)],
)
...
def test_kv_and_all_fixed_reserves_are_charged():
    ...
    with pytest.raises(ValueError, match="No memory-feasible partition"):
        partition_layers(
            _costs(8),
            num_layers=8,
            memory_profile=_profile([replace(d, budget_bytes=24) for d in devices]),
        )
...
def test_head_follows_final_selected_rank_and_single_rank_has_both_endpoints():
    ...
    with pytest.raises(ValueError, match="No memory-feasible partition"):
        partition_layers(
            _costs(8, (1, 1, 1)), num_layers=8, max_pp_size=1, memory_profile=memory
        )


def test_nonuniform_layer_memory_uses_actual_interval():
    memory = _profile([_device(0, 10, [8, 1, 1, 1]), _device(1, 10, [8, 1, 1, 1])])
...
def test_all_tp_devices_must_fit_not_just_tp_zero():
    memory = _profile(
        [
            _device(0, 100, [1] * 8, tp=0),
            _device(0, 3, [1] * 8, tp=1),
            _device(1, 100, [1] * 8, tp=0),
            _device(1, 5, [1] * 8, tp=1),
        ],
        tp=2,
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("budget_bytes", 1.5),
        ("runtime_bytes", True),
        ("graph_bytes", float("nan")),
        ("layer_weights_bytes", (1, -1)),
        ("layer_kv_bytes", (1,)),
    ],
)
...
def _write_traces(path, memory, tp_size=None):
    ...
    for r in range(memory.pp_size):
        ...
        record = dict(
            ...
            compute_wall_ms=end - start,
            send_service_ms=1 if r < memory.pp_size - 1 else None,
            send_ms=1 if r < memory.pp_size - 1 else None,
            send_transfer_ms=1 if r < memory.pp_size - 1 else None,
            tp_size=memory.tp_size if tp_size is None else tp_size,
        )
        records_by_rank[r] = [record]
        write_trace(path / f"pp_stage_pp{r}_tp0.jsonl", [record])
    write_fit_profile(path / "fit", records_by_rank)
...
def _engine_config(memory):
    ...
    return SimpleNamespace(
        model_config=SimpleNamespace(
            **{
                k: scope[k]
                for k in ("model", "revision", "dtype", "quantization", "max_model_len")
            },
            get_total_num_hidden_layers=lambda: memory.num_layers,
        ),
        cache_config=SimpleNamespace(
            cache_dtype=scope["kv_cache_dtype"],
        ...
        parallel_config=SimpleNamespace(
            tensor_parallel_size=memory.tp_size, pipeline_parallel_size=memory.pp_size
        ),
    )
...
def test_missing_memory_stops_live_run_before_factory(tmp_path):
    called = []
    with pytest.raises(ValueError, match="memory feasibility requires"):
        profile_pp_partition(dump_dir=tmp_path, llm_factory=lambda: called.append(True))
    assert not called
    ...
    for _ in range(25):
        ...
        for size in range(1, p + 1):
            for cuts in itertools.combinations(range(1, n), size - 1):
                ...
                if fits:
                    cost = max(
                        count * speeds[r] + (1 if r else 0) + (1 if r < size - 1 else 0)
...
def test_live_memory_profile_is_saved_and_replay_is_equivalent(tmp_path):
    ...
    llm = SimpleNamespace(
        llm_engine=SimpleNamespace(
            vllm_config=_engine_config(memory),
            engine_core=SimpleNamespace(shutdown=lambda: events.append("shutdown")),
        ),
        generate=lambda *a, **kw: _write_traces(tmp_path, memory),
        collective_rpc=lambda method, args: None,
    )
...
@pytest.mark.parametrize(
    "override", ["kv_cache_memory_bytes", "num_gpu_blocks_override"]
)
...
def test_memory_observation_counts_storage_once_and_reports_kv_included(monkeypatch):
    import torch
    ...
    model.head = model.embed  # tied endpoint storage must not be counted twice
    monkeypatch.setattr(
    ...
    for name, value in [
        ("memory_allocated", 50),
        ("memory_reserved", 60),
        ("max_memory_allocated", 70),
        ("max_memory_reserved", 80),
    ]:
        monkeypatch.setattr(torch.cuda, name, lambda device, value=value: value)
    result = observe_worker_memory(
        SimpleNamespace(
            device=torch.device("cuda:0"), model_runner=SimpleNamespace(model=model)
        )
    )
    assert result["layer_storage_bytes"] == {"0": 16}
    assert result["non_layer_storage_bytes"] == 24
    assert result["peak_allocated_bytes"] == 70
    assert result["includes_kv_pool"] is True
